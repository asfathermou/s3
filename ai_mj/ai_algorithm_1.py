...
class AIAlgorithm:
    ...
    def set_scenario(self, scenario, graph):
        self._scenario = scenario
        self._graph = graph

    def set_seat_ids(self, seat_id):
    ...
    def _adjust_initial_positions(self, battle_state):
        ...
        """
        adjust_list = []
        # adjust_list.append({'operator_id': 110801, 'position': '1,4'})
        # adjust_list.append({'operator_id': 110802, 'position': '2,1'})
        # adjust_list.append({'operator_id': 110803, 'position': '3,1'})
        # adjust_list.append({'operator_id': 110804, 'position': '1,1'})
        # for operator in battle_state.our_operators:
        #     self._adjust_operator_initial_position(operator, battle_state, adjust_list)

        # adjust_list.append({'operator_id': 210403, 'position': '1,4'})
        # adjust_list.append({'operator_id': 110802, 'position': '2,1'})
        # adjust_list.append({'operator_id': 110803, 'position': '3,1'})
        # adjust_list.append({'operator_id': 110804, 'position': '1,1'})
    ...
    def target_id_transform(self, src_id):
        ...
        for id in target_id_list:
            # 如果攻击目标是单算子，直接加到过滤列表中
            if str(id).startswith(str(2108)):
                target_id_list_filter.append(id)
            # 如果攻击目标中有编队，计算上下部分算子列表
            else:
                member = self.df_formation.at[id, 'operator_id']
                defense_operator_list = []
                for item in member.split(','):
                    defense_operator_list.append(int(item))
                select_result = self._sdk.operator_seat.antiShipMissilesTargetSelect(self._round, -1, [src_id], id,
                                                                                     defense_operator_list)
                select_target_id_list = select_result['select_target_ids']
                print("攻击方{}从编队{}中选择的算子id是：{}".format(src_id, defense_operator_list, select_target_id_list))
                # 如果选出来的被攻击算子个数仍大于1个，则随机选择一个算子做为攻击列表中的算子
                if len(select_target_id_list) >= 1:
                    target_id_list_filter.append(random.choice(select_target_id_list))

        return target_id_list_filter

    def select_best_target_id(self, target_id_list):
        """
        将可以攻击的目标id排序： 按有弹有损算子-有弹无损算子-无弹有损算子-无弹无损算子顺序
        @param target_id_list: 被攻击方id
        @return 排序后的list
        """
        df_target = self.df_operator.loc[target_id_list]
        df_target = df_target.sort_values(by=['asm_num', 'state'], ascending=(False, False))
        index_list = df_target.index.tolist()
        return index_list
    ...
    def _game_over(self, battle_state):
        ...
        self._logger.print(self._round,
                           "红方净胜分：{}".format((scores[0] + scores[1] + scores[2]) - (scores[3] + scores[4] + scores[5])))
        ...
        self._logger.print(self._round,
                           "蓝方净胜分：{}".format((scores[3] + scores[4] + scores[5]) - (scores[0] + scores[1] + scores[2])))
